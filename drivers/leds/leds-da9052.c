...
//printk("led->type %d\n",led->type);
		...
		else
		{
			...
			if(led->blink_on > 1000)	/*2秒間隔で40ms点灯*/
			{
				msg.data = (msg.data & 0x1f) | 0xc0;
				is_led6_blink = 2;
			}
			else if(led->blink_on) /*1秒間隔で40ms点灯*/
			{
...
#ifdef CONFIG_MACH_MX53_BEJ2
#if 0 //本来はここで初期化すべきだが、今回は、バッテリー側でLED制御しているのでコメントアウト
...
//		led_dat->cdev.blink_set = da9052_blink_set;
		...
		ret = device_create_file(led_dat->cdev.dev, &dev_attr_flash_on);
		if (ret) {
			dev_err(&pdev->dev, "failed to register flash led %d\n",
					led_dat->id);
			goto err_register;
