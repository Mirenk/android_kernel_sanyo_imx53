...
static int mxc_v4l2_s_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	pr_debug("In MVC:mxc_v4l2_s_ctrl id:%x\n",c->id);
...
//=== Sanyo CE
	...
	case   V4L2_CID_POWER_LINE_FREQUENCY:
		if (cam->sensor) {
			...
			printk("ERROR: V4L2_CID_POWER_LINE_FREQUENCY is v4l2 capture: slave not found!\n");
			ret = -ENODEV;
		}
		break;
//=== Sanyo 
...
//-- Sanyo  CE //サスペンド復帰時に初期化されないため追加
	if( parm->parm.capture.capturemode < 10 ) {
		if( v4l_close_flag != 0 ) {
			currentparm.parm.capture.capturemode = -1;
			 v4l_close_flag = 0;
		}
	} else {
			printk("mxc_v4l2_s_param Size not change");
			 v4l_close_flag = 0;
			goto Skip1;
	}
//-- Sanyo  CE
...
//--- Sanyo CE 画面サイズが同一でフレームレートが異なる場合に設定が更新されないため
#if 0
	if (parm->parm.capture.capturemode == currentparm.parm.capture.capturemode) {
		return 0;
	}
#endif
	if (parm->parm.capture.capturemode == currentparm.parm.capture.capturemode) {	//キャプチャーサイズが同一
		if (parm->parm.capture.timeperframe.denominator == currentparm.parm.capture.timeperframe.denominator) {	//フレームレートが同一
			return 0;
		}
	}
//--- Sanyo CE 画面サイズが同一でフレームレートが異なる場合に設定が更新されないため
...
//	cam->streamparm.parm.capture.capturemode = 0;
	cam->streamparm.parm.capture.capturemode = -1;
//-- Sanyo CE
...
void set_rnss2e1_suspend_flag(int);
void set_sr200pc20_suspend_flag(int);
#endif
