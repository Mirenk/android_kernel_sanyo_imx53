...
/* Add Sanyo CE*/
extern int da9053_rtc_set_time(struct rtc_time *rtc_tm);
extern int da9053_rtc_get_time(struct rtc_time *rtc_tm);
extern int da9053_rtc_chk_valid(void);
/* Add Sanyo CE End*/
...
/* Add Sanyo CE*/
	lpcr = __raw_readl(ioaddr + SRTC_LPCR);
	lpsr = __raw_readl(ioaddr + SRTC_LPSR);
/* Add Sanyo CE End*/
...
/* Add Sanyo CE*/
	if((da9053_rtc_chk_valid() == 1) && (lpcr == 0) && (lpsr == 0))
	{
		struct rtc_time tm;
		da9053_rtc_get_time(&tm);
		mxc_rtc_set_time(&pdev->dev, &tm);
	}
/* Add Sanyo CE End*/
