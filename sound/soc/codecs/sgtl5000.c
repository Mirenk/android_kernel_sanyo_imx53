...
//	uinfo->value.integer.max = 0xfc - 0x3c;
	uinfo->value.integer.max = 0xfc - DAC_MAX_VOLUME;
	return 0;
}
...
//Max volume -6dB
//	l = l < 0x3c ? 0x3c : l;
	l = l < DAC_MAX_VOLUME ? DAC_MAX_VOLUME : l;
	l = l > 0xfc ? 0xfc : l;
//	r = r < 0x3c ? 0x3c : r;
	r = r < DAC_MAX_VOLUME ? DAC_MAX_VOLUME : r;
	r = r > 0xfc ? 0xfc : r;
	l = 0xfc - l;
	r = 0xfc - r;
...
//	l = l > 0xfc - 0x3c ? 0xfc - 0x3c : l;
	l = l > 0xfc - DAC_MAX_VOLUME ? 0xfc - DAC_MAX_VOLUME : l;
	r = r < 0 ? 0 : r;
//	r = r > 0xfc - 0x3c ? 0xfc - 0x3c : r;
	r = r > 0xfc - DAC_MAX_VOLUME ? 0xfc - DAC_MAX_VOLUME : r;
	l = 0xfc - l;
	r = 0xfc - r;
...
static int hp_info_volsw(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 2;
	uinfo->value.integer.min = 0x7f;
	uinfo->value.integer.max = 0x18;
	return 0;
}

static int hp_get_volsw(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int reg, l, r;

	reg = sgtl5000_read(codec, SGTL5000_CHIP_ANA_HP_CTRL);
	l = (reg & SGTL5000_HP_VOL_LEFT_MASK) >> SGTL5000_HP_VOL_LEFT_SHIFT;
	r = (reg & SGTL5000_HP_VOL_RIGHT_MASK) >> SGTL5000_HP_VOL_RIGHT_SHIFT;
...
static int hp_put_volsw(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int reg, l, r;
	...
	reg = l << SGTL5000_HP_VOL_LEFT_SHIFT |
	    r << SGTL5000_HP_VOL_RIGHT_SHIFT;

	sgtl5000_write(codec, SGTL5000_CHIP_ANA_HP_CTRL, reg);
...
static int sgtl5000_resume(struct platform_device *pdev)
{
	...
	if (codec->suspend_bias_level == SND_SOC_BIAS_STANDBY)
	{
		sgtl5000->need_clk_for_bias = 0;
		sgtl5000_clock_gating(codec, 0);
	}
...
static int sgtl5000_probe(struct platform_device *pdev)
{
	...
	/* TODO: initialize DAP */
#else
	sgtl5000_write(codec, SGTL5000_DAP_MAIN_CHAN, 0x8000);//100%
	sgtl5000_write(codec, SGTL5000_DAP_MIX_CHAN, 0x8000);//100%

	sgtl5000_write(codec, SGTL5000_DAP_CTRL, SGTL5000_DAP_MIX_EN | SGTL5000_DAP_EN);
#endif
	...
	/* HP Volume */
//Max volume -6dB
//	sgtl5000_write(codec, SGTL5000_CHIP_ANA_HP_CTRL, 0x2222);
	sgtl5000_write(codec, SGTL5000_CHIP_ANA_HP_CTRL, 0x2e2e);

//Max volume -6dB
	/* DAC Volume*/
	sgtl5000_write(codec, SGTL5000_CHIP_DAC_VOL, DAC_MAX_VOLUME | DAC_MAX_VOLUME << 8);
