			    ...
			    struct snd_soc_dai *dai)
{
	...
	if(channels == 2)//ステレオ
	{
		snd_soc_update_bits(codec, WM8978_OUTPUT_CONTROL, 0x60, 0x60);//DACL2RMIX,DACR2LMIX=1
		snd_soc_update_bits(codec, WM8978_RIGHT_MIXER_CONTROL, 0x01, 0x01);//DACR2RMIX=1
	}
	else//モノラル
	{
		snd_soc_update_bits(codec, WM8978_OUTPUT_CONTROL, 0x60, 0x40);//DACL2RMIX=1
		snd_soc_update_bits(codec, WM8978_RIGHT_MIXER_CONTROL, 0x01, 0x00);//DACR2RMIX=0
	}
	snd_soc_write(codec, WM8978_BEEP_CONTROL, 0x10);//モノラルスピーカー
	snd_soc_update_bits(codec, WM8978_POWER_MANAGEMENT_1, 0x100, 0x100);
	snd_soc_update_bits(codec, WM8978_OUTPUT_CONTROL, 0x04, 0x04);//SPKBOOST=1
#endif
