/*
 * imx-3stack-wm8978.c  --  i.MX 3Stack Driver for Freescale SGTL5000 Codec
 *
...
static unsigned int wm8978_rates[] = {
	0,
	32000,
	44100,
	48000,
...
//	snd_soc_dai_set_sysclk(codec_dai, WM8978_MCLK, priv->sysclk, 0);
	snd_soc_dai_set_sysclk(codec_dai, WM8978_PLL, priv->sysclk, 0);

#if WM8978_SSI_MASTER
...
/*
 * imx_3stack WM8978 audio DAI opserations.
 */
...
static void imx_3stack_init_dam(int ssi_port, int dai_port)
{
	...
	/* WM8978 uses SSI1 or SSI2 via AUDMUX port dai_port for audio */
...
static int wm8978_jack_func;
static int wm8978_spk_func;
static int wm8978_line_in_func;
...
static int wm8978_get_jack(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = wm8978_jack_func;
	return 0;
}

static int wm8978_set_jack(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (wm8978_jack_func == ucontrol->value.enumerated.item[0])
		return 0;

	wm8978_jack_func = ucontrol->value.enumerated.item[0];
	if (wm8978_jack_func)
...
static int wm8978_get_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = wm8978_spk_func;
	return 0;
}

static int wm8978_set_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (wm8978_spk_func == ucontrol->value.enumerated.item[0])
		return 0;

	wm8978_spk_func = ucontrol->value.enumerated.item[0];
	if (wm8978_spk_func)
...
static int wm8978_get_line_in(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = wm8978_line_in_func;
	return 0;
}

static int wm8978_set_line_in(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (wm8978_line_in_func == ucontrol->value.enumerated.item[0])
		return 0;

	wm8978_line_in_func = ucontrol->value.enumerated.item[0];
	if (wm8978_line_in_func)
...
static const struct snd_kcontrol_new wm8978_machine_controls[] = {
	SOC_ENUM_EXT("Jack Function", wm8978_enum[0], wm8978_get_jack,
		     wm8978_set_jack),
	SOC_ENUM_EXT("Speaker Function", wm8978_enum[1], wm8978_get_spk,
		     wm8978_set_spk),
	SOC_ENUM_EXT("Line In Function", wm8978_enum[1], wm8978_get_line_in,
		     wm8978_set_line_in),
};
...
static const struct soc_enum asrc_enum[] = {
	SOC_ENUM_SINGLE_EXT(4, asrc_function),
};
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	asrc_ssi_data.output_sample_rate = wm8978_rates[asrc_func];
...
static int imx_3stack_wm8978_init(struct snd_soc_codec *codec)
{
	int i, ret;
	...
	for (i = 0; i < ARRAY_SIZE(wm8978_machine_controls); i++) {
		ret = snd_ctl_add(codec->card,
				  snd_soc_cnew(&wm8978_machine_controls[i],
					       codec, NULL));
		if (ret < 0)
			return ret;
	}
...
static int __devinit imx_3stack_wm8978_probe(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_3stack_priv *priv = &card_priv;
	struct snd_soc_dai *wm8978_cpu_dai = 0;
//	struct wm8978_setup_data *setup;
	...
	if (plat->src_port == 2)
		wm8978_cpu_dai = imx_ssi_dai[2];
	else if (plat->src_port == 1)
		wm8978_cpu_dai = imx_ssi_dai[0];
	else if (plat->src_port == 7)
		wm8978_cpu_dai = imx_ssi_dai[4];


	imx_3stack_dai.cpu_dai = wm8978_cpu_dai;
	...
	/* The WM8978 has an internal reset that is deasserted 8 SYS_MCLK
...
#if 0
	setup = kzalloc(sizeof(struct wm8978_setup_data), GFP_KERNEL);
	if (!setup) {
		pr_err("%s: kzalloc wm8978_setup_data failed\n", __func__);
		goto err_card_reg;
	}
...
static int imx_3stack_wm8978_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_3stack_priv *priv = &card_priv;
...
static struct platform_driver imx_3stack_wm8978_audio_driver = {
	.probe = imx_3stack_wm8978_probe,
	.remove = imx_3stack_wm8978_remove,
	.driver = {
...
static int __init imx_3stack_init(void)
{
	...
	ret = platform_driver_register(&imx_3stack_wm8978_audio_driver);
	if (ret)
		return -ENOMEM;
...
{
	platform_driver_unregister(&imx_3stack_wm8978_audio_driver);
...
MODULE_DESCRIPTION("WM8978 Driver for i.MX 3STACK");
MODULE_LICENSE("GPL");
